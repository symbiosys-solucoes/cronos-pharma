...
import br.symbiosys.solucoes.cronospharma.entidades.cronos.*
...
@Service
class CloseUPService (private val pedidoPalmRepository: PedidoPalmRepository,
                      ...
                      private val retornoNotaCLOSEUPRepository: RetornoNotaCLOSEUPRepository,
) {
    private val logger = LoggerFactory.getLogger(CloseUPService::class.java)
    ...
    fun gerarArquivoDeRetornoPedido(id: Long) {
        ...
        val diretorio = diretoriosRepository.findAll().filter { dir -> dir.tipoIntegracao.equals(TipoIntegracao.CLOSEUP)}.firstOrNull()
        if (diretorio == null) {
            logger.error("Nao foi encontrado configuracao para a integracao ${TipoIntegracao.CLOSEUP.name}")
            throw DiretorioNotFoundException("Nao foi encontrado configuracao para a integracao ${TipoIntegracao.CLOSEUP.name}")
        }

        val retorno =  Pedido().gerarRetorno(cnpj, pedidoPalm, diretorio)
        val arquivo = Arquivo()
    ...
    fun gerarArquivoRetornoNF(id: Long) {
        ...
        val retornos = retornoNotaCLOSEUPRepository.findRetornos(listOf(pedidoPalm.IdPedidoPalm!!), force = true)
        val arquivo = Arquivo()
